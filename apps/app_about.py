        ...
        html.Div(
          [
            ...
            dcc.Markdown(dedent('''
              ...
              _Homology-directed repair (HDR)_ is one DNA repair pathway that has been in the spotlight. In HDR, scientists can introduce a DNA template that looks similar to, but is not exactly the same as, the DNA surrounding the CRISPR cut. The cell will use this DNA template to repair the cut. The result is controllable editing of DNA at a precise location in the genome into user-specified DNA.

              It turns out, however, that HDR is often less efficient than another DNA repair pathway called _non-homologous end-joining (NHEJ)_. In contrast to HDR which can be used to introduce user-specified edits, NHEJ has not been widely recognized as controllable. In the context of not only a traumatic DNA double-strand break, but CRISPR which will repeatedly cut the DNA, NHEJ can be thought of as a "willy-nilly", no-holds-barred effort to repair the cut into a form that CRISPR can no longer recut. The results are "ugly" insertions and deletions which are hugely diverse and heterogeneous in different cells. While HDR has been a golden child of genome editing, NHEJ has been viewed as undesirable noise that is unfortunately more efficient than HDR.
